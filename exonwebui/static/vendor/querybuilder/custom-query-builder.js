...
QueryBuilder.DEFAULTS = {
    ...
    icons: {
        add_group: 'fa fas fa-plus-square',
        add_rule: 'fa fas fa-plus-circle',
        remove_group: 'fa fas fa-minus-square',
        remove_rule: 'fa fas fa-minus-circle',
        error: 'fa fas fa-exclamation-triangle'
    }
};
    ...
    <div class="btn-group group-conditions"> \
      {{~ it.conditions: condition }} \
        <label class="btn btn-sm btn-primary"> \
          <input class="form-check-input" type="radio" name="{{= it.group_id }}_cond" value="{{= condition }}"> {{= it.translate("conditions", condition) }} \
        </label> \
      {{~}} \
    </div> \
...
{{ var optgroup = null; }} \
<select class="custom-select custom-select-sm scroll" name="{{= it.rule.id }}_filter"> \
...
{{?}} \
{{ var optgroup = null; }} \
<select class="custom-select custom-select-sm scroll {{? it.operators.length === 1 }}hide{{?}}" name="{{= it.rule.id }}_operator"> \
...
{{ var optgroup = null; }} \
<select class="custom-select custom-select-sm scroll" name="{{= it.name }}" {{? it.rule.filter.multiple }}multiple{{?}}> \
        ...
        switch (filter.input) {
            ...
            case 'checkbox':
                Utils.iterateOptions(filter.values, function(key, val) {
                    h += '<label ' + c + '><input class="form-check-input" type="' + filter.input + '" name="' + name + '" value="' + key + '">' + val + '</label>';
                });
                break;
            ...
            case 'textarea':
                h += '<textarea class="form-control form-control-sm scroll" name="' + name + '"';
 ...
 * @param {object} [options]
 * @param {string} [options.icon='fa fas fa-info-sign']
 ...
 * @param {object} [options]
 * @param {string} [options.icon='fa fas fa-random']
    ...
    if (!options.disable_template) {
        this.on('getGroupTemplate.filter', function(h) {
            ...
            $h.find(Selectors.condition_container).after(
                '<button type="button" class="btn btn-sm btn-light border" data-invert="group">' +
        ...
        if (options.display_rules_button && options.invert_rules) {
            this.on('getRuleTemplate.filter', function(h) {
                ...
                $h.find(Selectors.rule_actions).prepend(
                    '<button type="button" class="btn btn-sm btn-light border" data-invert="rule">' +
                    '<i class="' + options.icon + '"></i> ' + self.translate('invert') +
                    '</button>'
                );
                h.value = $h.prop('outerHTML');
 ...
 * @param {object} [options]
 * @param {string} [options.icon_checked='fa fas fa-checked']
 * @param {string} [options.icon_unchecked='fa fas fa-unchecked']
 */
QueryBuilder.define('not-group', function(options) {
    ...
    if (!options.disable_template) {
        this.on('getGroupTemplate.filter', function(h) {
            ...
            $h.find(QueryBuilder.selectors.condition_container).prepend(
                '<button type="button" class="btn btn-sm btn-secondary" data-not="group">' +
    ...
    icon_unchecked: 'fa fas fa-square',
    icon_checked: 'fa fas fa-check-square',
    disable_template: false
});
 ...
 * @param {string} [options.icon='fa fas fa-sort']
